# Kaggle Integration — Локальная работа с GPU

Интеграция с Kaggle для обучения моделей на GPU без перехода на сайт.

Данные хранятся в Yandex Cloud и управляются через DVC. Kaggle Datasets не используется.
...
```bash
cp .env.example .env
# Заполните KAGGLE_USERNAME и KAGGLE_KEY
```
...
Данные скачиваются напрямую из Yandex Cloud S3 в начале ноутбука через `boto3`. Требуется `enable_internet=True` при отправке kernel.
...
### Ошибка: `400 Bad Request - code_file not found`

**Решение:** Проверьте, что `kernel-metadata.json` создан корректно.

### Ошибка: `TimeoutError: Kernel did not complete within 7200s`

**Решение:** Увеличьте `--timeout` или проверьте ноутбук на ошибки.

### Ошибка: `Access Denied / 403 Forbidden`
