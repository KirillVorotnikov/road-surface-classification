...
class TestSpecAugment:
    ...
    def test_mask_values(self, sample_spectrogram):
        ...
        if masked_positions.any():
            ...
            masked_rows = np.where(masked_positions.any(axis=1))[0]
            for row in masked_rows:
                row_data = augmented[row, :]
                # Проверяем что есть участок с постоянными значениями
                unique_vals = np.unique(row_data)
                assert len(unique_vals) < len(row_data), (
                    "Маска не заполнена константой"
                )
